...
//
// It is consumed by hanzoai/ingress (the edge router) as a ForwardAuth middleware:
...
// waitlistHeaders are the guard's OWN response headers (written on allow).
...
var waitlistHeaders = []string{"X-Waitlist-Guard", "X-Waitlist-Approved"}

func main() {
	// DRY generator: emit the ingress `waitlist-strip` middleware from the SINGLE
	// source of truth (authz.Headers + waitlistHeaders) so
...
func (c *config) handleVerify(w http.ResponseWriter, r *http.Request) {
	...
	// writes. Delete every client-supplied copy up front so a forged inbound
...
// X-Forwarded-* headers (ingress forward-auth sets these).
...
// stripWaitlistHeaders deletes the guard's OWN response headers from an inbound
...
func printStripMiddleware(w io.Writer) {
	...
	fmt.Fprintln(w, "# Source of truth: authz.Headers + waitlistHeaders.")
	...
	names := append(append([]string{}, authz.Headers...), waitlistHeaders...)
	for _, h := range names {
		fmt.Fprintf(w, "          %s: \"\"\n", h)
	}
}
